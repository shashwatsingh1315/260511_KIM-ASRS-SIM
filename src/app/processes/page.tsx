    ...
    <div className="space-y-6">
      ...
      {identityProcesses.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-gray-300">Identity-Change Processes (BOMs)</h2>
          {identityProcesses.map((p) => (
            <div key={p.id} className="flex justify-between items-center p-3 border border-gray-800 rounded-lg bg-gray-900/50">
              <div>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {editingProcess?.bom_inputs?.map((input, i) => (
              <div key={i} className="flex items-center gap-3 bg-gray-950 p-3 rounded border border-gray-800">
                ...
                <button onClick={() => removeBomInput(i)} className="text-red-400 hover:text-red-300 ml-2">✕</button>
              </div>
            ))}
          ...
          <div className="flex justify-between pt-4">
            <button onClick={addBomInput} className="text-sm text-indigo-400 hover:text-indigo-300">
              + Add Component
            </button>
            <div className="space-x-3">
              <button onClick={() => setEditingProcess(null)} className="text-sm text-gray-400 hover:text-white">
                Cancel
              </button>
              <button onClick={saveBOM} className="text-sm bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded">
                Save BOM
              </button>
            </div>
          </div>
