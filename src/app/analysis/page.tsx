...
export default function AnalysisPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!result ? (
        <div className="p-12 text-center text-gray-500 border border-dashed border-white/10 rounded-2xl bg-white/[0.02]">
          Click "Run Engine" to analyze your current network.
        </div>
      ) : !result.is_valid ? (
        <div className="bg-red-950/20 border border-red-900/50 rounded-2xl p-6 space-y-4 backdrop-blur-md">
        ...
        <div className="space-y-6">
          ...
          <CollapsibleSection title="Critical Alerts" badge={result.alerts.length} defaultOpen={true}>
            {result.alerts.length === 0 ? (
              <div className="p-4 bg-green-950/20 border border-green-900/30 rounded-xl text-sm text-green-400">
                ✅ No bottlenecks or violations found. The network is capable.
              </div>
            ) : (
              <div className="space-y-2">
                {result.alerts.map((a, i) => (
                  <div key={i} className={`p-4 border rounded-xl text-sm backdrop-blur-sm ${a.severity === "error" ? "bg-red-950/10 border-red-900/30 text-red-400" : "bg-yellow-950/10 border-yellow-900/30 text-yellow-400"}`}>
                    ...
                    return (
                      <tr key={i} className="hover:bg-white/[0.03] transition-colors">
                        <td className="px-4 py-3 text-gray-200">
                          {mat?.name} <span className="text-xs text-gray-500 ml-1">({mat?.code})</span>
                        </td>
                        <td className="px-4 py-3 text-gray-400 italic">
                          {proc?.name} {sd.is_reject_branch && <span className="text-red-400 text-[10px] ml-1 uppercase">Reject</span>}
                        </td>
                        <td className="px-4 py-3 text-right text-gray-200">{sd.throughput_units_pph.toFixed(1)}</td>
                        <td className="px-4 py-3 text-right font-mono text-indigo-400">{sd.throughput_pallets_pph}</td>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                  {result.station_checks.map((sc, i) => (
                    <tr key={i} className="hover:bg-white/[0.03] transition-colors">
                      <td className="px-4 py-3 text-gray-200">{sc.station_name}</td>
                      <td className={`px-4 py-3 text-right font-medium ${sc.throughput_bottleneck ? "text-red-400" : "text-green-400"}`}>
                      ...
                      <td className={`px-4 py-3 text-right font-medium ${sc.holding_bottleneck ? "text-red-400" : "text-green-400"}`}>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                  {result.edge_checks.map((ec, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-white/[0.03] transition-colors">
                        <td className="px-4 py-3 text-gray-200 font-medium">{ec.edge_name}</td>
                        <td className="px-4 py-3 text-xs text-gray-500 uppercase tracking-wider">{from} → {to}</td>
                        <td className="px-4 py-3 text-right text-gray-200">{ec.throughput_required_pph.toFixed(1)}</td>
                        <td className="px-4 py-3 text-right text-gray-400">{ec.throughput_capacity_pph}</td>
                        <td className="px-4 py-3 text-right">
                          <div className="flex flex-col items-end gap-1">
                            <span className={`text-sm font-bold ${ec.bottleneck ? "text-red-400" : "text-indigo-400"}`}>
          ...
          <CollapsibleSection title="Cell Footprint">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {result.cell_checks.map((cc, i) => (
                <div key={i} className="p-4 rounded-xl border border-white/10 bg-white/[0.02] flex justify-between items-center">
                  <div>
                    <h3 className="text-gray-200 font-medium">{cc.cell_name}</h3>
                    <p className="text-xs text-gray-500 mt-1">Available: {(cc.footprint_available_mm2 / 1e6).toFixed(1)} m²</p>
                  </div>
                  <div className="text-right">
                    <div className={`text-lg font-bold ${cc.footprint_bottleneck ? "text-red-400" : "text-green-400"}`}>
                      {(cc.footprint_required_mm2 / 1e6).toFixed(1)} m²
                    </div>
                    <div className="text-[10px] text-gray-500 uppercase tracking-widest">Required</div>
                  </div>
                </div>
              ))}
            </div>
...
function CollapsibleSection({ title, children, badge, defaultOpen = false }: { title: string; children: React.ReactNode; badge?: number; defaultOpen?: boolean }) {
  ...
  return (
    <div className="border border-white/10 rounded-2xl overflow-hidden bg-[#1c1c1e]/40 backdrop-blur-md shadow-sm">
      <button 
      ...
      >
        <div className="flex items-center gap-3">
          ...
          {badge !== undefined && (
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${badge > 0 ? "bg-red-500/20 text-red-400 border border-red-500/30" : "bg-green-500/20 text-green-400 border border-green-500/30"}`}>
              {badge}
            </span>
          )}
        </div>
