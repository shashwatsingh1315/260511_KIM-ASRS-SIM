# ASRS-Choker — Plan Part 3: Validation Engine

> **Prerequisite:** Complete plans 01 and 02 first.
> **Goal:** Build `src/lib/validation.ts` — 15 checks that must ALL pass before any analysis can run.

---

## What is Validation?

Before computing any demand or WIP, we must confirm the data is complete and consistent. If any check fails, the user sees the error and the engine is blocked.

All 15 rules come from Section 9 of `logic-spec-v1.0.md`.

---

## Task 4: Create the Validation Module

**Step 4.1** — Create `src/lib/validation.ts`:
...
```powershell
git add .
git commit -m "feat: validation engine (all 15 rules)"
```
...
- [ ] `git commit -m "feat: validation engine (all 15 rules)"` done

**➡ Continue with: `plan-04-analysis-engine.md`**
